...
export default function ThemeToggle({ onHero = false }: { onHero?: boolean }) {
  ...
  const stroke = onHero ? "#FFFFFF" : "var(--text-primary)";
  const borderColor = onHero ? "rgba(255,255,255,0.35)" : "var(--border)";

  return (
    <button
      onClick={toggleTheme}
      className="relative w-9 h-9 flex items-center justify-center transition-all duration-200 hover:scale-105"
      style={{ background: "transparent", border: `1px solid ${borderColor}`, borderRadius: "4px" }}
      ...
      <AnimatePresence mode="wait">
        {theme === "light" ? (
          <motion.svg key="sun" initial={{ rotate: -90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: 90, opacity: 0 }} transition={{ duration: 0.25 }}
            className="w-[18px] h-[18px]" fill="none" stroke={stroke} viewBox="0 0 24 24" strokeWidth={2}>
