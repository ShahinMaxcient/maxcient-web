  ...
  return (
    <div style={{ position: "absolute", width: size, height: size, perspective: 600, ...pos, animation: anim }}>
      <div style={{ position: "relative", width: "100%", height: "100%", transformStyle: "preserve-3d", animation: "ed-cube-spin 30s linear infinite" }}>
        {faces.map((t, i) => (
          <div key={i} style={{ position: "absolute", inset: 0, border: "1.5px solid rgba(255,255,255,0.30)", background: i === 3 ? "rgba(255,255,255,0.18)" : "transparent", borderColor: i === 3 ? "rgba(255,255,255,0.5)" : "rgba(255,255,255,0.30)", transform: t }} />
        ))}
      </div>
    </div>
  );
  ...
  return (
    <section className="relative overflow-hidden" style={{ background: "linear-gradient(135deg, var(--primary-dark) 0%, var(--primary) 50%, var(--primary-light) 100%)", padding: "120px 0" }}>
      ...
      <div className="relative max-w-[1100px] mx-auto px-5 sm:px-8 text-center">
        ...
        <p className="mx-auto mt-7 leading-relaxed" style={{ fontSize: "1.15rem", color: "rgba(255,255,255,0.78)", maxWidth: "540px" }}>{subtitle}</p>
        <div className="mt-12 flex flex-col sm:flex-row gap-4 justify-center">
          <Link href="/request-a-consultation" className="inline-flex items-center justify-center gap-3 px-9 py-4 transition-all duration-200 hover:-translate-y-0.5"
            style={{ background: "#FFFFFF", color: "var(--primary-dark)", fontWeight: 600, fontSize: "15px", borderRadius: "4px" }}
          >
            Book a consultation
